    ...
    config:{
        ...
        control:{
            ...
            roomserviceView:{
                ...
                myCartRoomCommand:"onmyCartCommand",
                hMasterCommand: "onhMasterCommand"
            },
            descriptionView:{
            ...
            cartView:{
                ...
                hMasterCommand: "onhMasterCommand",
                continueShopping:"onContinueShoppingCommand"
            },
            galleryView: {
        ...
        loginStore.load({
            ...
            callback: function () {
                ...
                ///////////    Ext.Msg.alert("Welcome"+resNameValue);
                    ...
                    settingslocalStore.each(function (record) {
                        var resname = record.data.res_name;
                        var welcomeHtml = "<h3 class='welcome' style='text-align: center; color: #000000'>HI</h3><p>"+resname+"</p>";
...
*/
    ...
    oncheckOutCommand: function(){
        ...
        cartStore = Ext.create('Ext.data.Store', {
            model: "CrotusValet.model.ProductDetail"
        });
        cartStore.load();
        cartStore.sync();
        console.log('lc'+cartStore);
        var chkmsg = cartStore.getData().length;
        if(chkmsg != 0) {
            Ext.Msg.confirm('','Unbilled Items found in your cart, Are you sure to log out.', function(btn){
                if (btn === 'yes'){
        ...
        {
            Ext.Msg.confirm('','Are you sure to log out.', function(btn){
                if (btn === 'yes'){
    ...
    oncategoryCommand: function (list, target, index, record, evt, options) {
        ...
        productdetailView.setRecord(record);

        //var breadCrumb = this.getCategorydetailView();
        //var topCat = breadCrumb.down('#subcategory').getHtml();
        //console.log("top222222222222222"+topCat);

        var toGO =  this.getProductdetailView();
        localStore1 = Ext.create('Ext.data.Store', {
            model: "CrotusValet.model.ProductDetail"
        });
        localStore1.load();
        var countLocalStoreProducts = localStore1.getCount();
   ...
   //         productdetailView.down('#btnMyCart').setText(''+'('+countLocalStoreProducts+')');
        ...
        var prodName = record.get("prod_class_name");
        //console.log("prooooooooooo"+prodName);
        Ext.getCmp('product').setHtml('<h5><span class="guestName">'+prodName+'</span></h5>');
    ...
    launch: function () {
        ...
        localStore.load();
        var validsettings = localStore.getData().length;
        console.log("valid"+validsettings);
        if(validsettings==0){
            Ext.getCmp('checkout').hide();
            welcomeView.down('#lblResName').setHtml('<h5 class=\"GuestInfo\"><span class=\"guestName\">Guest</span></h5>');
        }else{
            Ext.getCmp('checkout').show();
            welcomeView.down('#lblResName').setHtml('<h5 class=\"GuestInfo\"><span class=\"guestName\">Guest</span></h5>');
            localStore.each(function (record) {
                var resname = record.data.res_name;
                var welcomeHtml = "<h5 class=\"GuestInfo\"><span class=\"pinkTxt\">Welcome </span><span class=\"guestName\">"+resname+"</span></h5>";
                //   var welcomeHtml = "<div class='slides' > <ul> <!-- slides --> <li>  <img  src='resources/images/pic1.jpg' alt='image01' /> <div>Guest Information</div></br><div >This app provides you a scope to know and utilise the facilities in our resort.</div> </li> </ul></div>" +resname;
                welcomeView.down('#lblResName').setHtml(welcomeHtml);
                console.log('kljlk'+resname);
            });
...
//    Ext.Viewport.animateActiveItem(productdetailView, this.slideLeftTransition);
    ...
    onroomSerCommand: function (list, target, index, record, evt, options) {
        ...
        var roomsr = record.get("rev_class_name");
        Ext.getCmp('subcategory').setHtml('<h5><span class="guestName">'+roomsr+'</span></h5>');
        //countx
        localStore = Ext.create('Ext.data.Store', {
            model: "CrotusValet.model.ProductDetail"
        });
    ...
    onaddCartCommand:function(){
        ...
        settingslocalStore.each(function (record) {
            ...
            var rec = {
                ...
                price_net: parseFloat(productDescriptionView.down('#txt4').getValue()).toFixed(2),
                menu_quantity: productDescriptionView.down('#spinQuantity').getValue(),
                //      total_price: productDescriptionView.down('#txt6').getValue(),
                tax_amt:  parseFloat(productDescriptionView.down('#txt7').getValue()).toFixed(2),
                ...
                total:((parseFloat(productDescriptionView.down('#txt4').getValue())+parseFloat(productDescriptionView.down('#txt7').getValue()))*(productDescriptionView.down('#spinQuantity').getValue())).toFixed(2),
    ...
    onbackToMasterViewCommand:function(){
        ...
        var toGO =  this.getDescriptionView();
        /*localStore = Ext.create('Ext.data.Store', {
            model: "CrotusValet.model.ProductDetail"
        });
        localStore.load();
    ...
    onContinueShoppingCommand:function(){
